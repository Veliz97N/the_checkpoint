...
const Catalogo_PaginaPrincipal = () => {
  ...
  const funcion_filtrar_busqueda_producto = (tipoBusqueda, valorBusqueda) => {
    ...
    if (tipoBusqueda === "0" && valorBusqueda !== "") {
      ...
      const productoFiltrados = productos.filter((producto) =>
        producto.categoria.toLowerCase().includes(valorBusqueda.toLowerCase())
      );
      setListaProductosFiltrado(productoFiltrados);
    } else if (tipoBusqueda === "2" && valorBusqueda !== "") {
      const productoFiltrados = productos.filter((producto) =>
        producto.codigodebarras.includes(valorBusqueda)
      );
      setListaProductosFiltrado(productoFiltrados);
    } else if (valorBusqueda === "" || listaProductosFiltrado.length === 0) {
      //Si el valor de busqueda es ''
          ...
          <table className="table">
            ...
            <tbody>
              <tr style={noactivopapi}>
                ...
                <td colspan="4">
                  <input
                    ...
                    onKeyPress={(e) => {
                      if (e.key === "Enter") {
                        funcion_filtrar_busqueda_producto(
                          indiceBuscarElemento,
                          valor
                        );
                      }
                    }}
              ...
              {listaProductosFiltrado.length >= 1
                ? listaProductosFiltrado.map((producto) => (
                    <Producto_en_tabla_catalogo producto={producto} />
                  ))
